...
{
    public class ElargedSWSynonyms : IReworder
    {
        ...
        "he'll he's her here here's hers herself him himself his how how's i i'd i'll i'm i've if in into is isn't it it's its itself let's me more most mustn't " +
        ...
        HashSet<string> _stopWords = new HashSet<string>();

        SynonymsQuotient _sq;

        public ElargedSWSynonyms(string filePath)
        {
            foreach (string elt in _data)
                _stopWords.Add(elt);

            _sq = new SynonymsQuotient(filePath);
        ...
        {
            return _stopWords.Contains(elt);
        }

        public string Map(string elt)
        {
            if (_stopWords.Contains(elt))
                return "";
            else
                return _sq.Map(elt);
        }
